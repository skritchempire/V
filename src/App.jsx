...
import { Zap, Droplet, Wind, Activity, Wrench, ArrowUpCircle, AlertTriangle, Users, PlusCircle, Heart, Train, Skull, ShieldAlert, DoorClosed, Map, Footprints, Radio, Sparkles, X, Swords, Martini, Sprout, ShoppingCart, Crosshair, Star, HelpCircle, Database, FlaskConical, Lock, Shield, Cpu, ShieldHalf, Syringe, Bomb, CarFront, CheckSquare, Rocket, ZapOff, Flame, Bug, Scale, Briefcase, Target, Hand, Settings, Save, RotateCcw, Download } from 'lucide-react';
        ...
        const ROOM_BACKGROUNDS = {
          living: '/assets/rooms/living.webp',
          generator: '/assets/rooms/generator.webp',
          filter: '/assets/rooms/filter.webp',
          reservoir: '/assets/rooms/reservoir.webp',
          farm: '/assets/rooms/farm.webp',
          medbay: '/assets/rooms/medbay.webp',
          bar: '/assets/rooms/bar.webp',
          armory: '/assets/rooms/armory.webp',
          warehouse: '/assets/rooms/warehouse.webp',
          lab: '/assets/rooms/lab.webp',
          garage: '/assets/rooms/garage.webp',
          turret: '/assets/rooms/turret.webp',
          radio: '/assets/rooms/radio.webp',
          guard: '/assets/rooms/guard.webp',
          project: '/assets/rooms/project.webp'
        };

        const DWELLER_SPRITES = {
          leader: '/assets/chars/leader.webp',
          normis: '/assets/chars/normis.webp',
          soy: '/assets/chars/soy.webp',
          skuf: '/assets/chars/skuf.webp',
          gigachad: '/assets/chars/gigachad.webp',
          mutant_worker: '/assets/chars/mutant_worker.webp',
          mutant_combat: '/assets/chars/mutant_combat.webp',
          mutant_psi: '/assets/chars/mutant_psi.webp',
          sick: '/assets/chars/sick.webp',
          cyborg: '/assets/chars/cyborg.webp',
          fallback: '/assets/chars/normis.webp'
        };
        ...
        const Dweller = ({ dweller, isOutside = false, colorClass = 'bg-stone-500', isPaused = false, roomType = 'none', isSelected = false, onClick = null }) => {
          ...
          useEffect(() => { setSpriteFailed(false); }, [dweller.id, dweller.trait, dweller.isSick, dweller.isCyborg]);
          ...
          const spritePath = dweller.isCyborg
            ? DWELLER_SPRITES.cyborg
            : dweller.isSick
            ? DWELLER_SPRITES.sick
            : DWELLER_SPRITES[dweller.trait] || DWELLER_SPRITES.fallback;
          ...
          return (
            ...
            >
              ...
              {!spriteFailed ? (
                <div className="relative flex flex-col items-center justify-end transition-transform duration-150 pointer-events-none" style={{ transform: `scaleX(${pos.dir})` }}>
                  <img
                    src={spritePath}
                    alt={dweller.name}
                    className="w-8 h-10 object-contain drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]"
                    loading="lazy"
                    onError={() => setSpriteFailed(true)}
                  />
                  {dweller.equipped === 'weapon' && <div className="absolute right-0 top-3 text-[10px]">🔫</div>}
                  {dweller.equipped === 'armor' && <div className="absolute inset-x-1 bottom-2 h-3 border border-stone-300/70 rounded bg-stone-600/30" />}
                  {dweller.equipped === 'stimpak' && <div className="absolute right-0 bottom-2 text-[10px]">💉</div>}
                </div>
              ) : (
                    ...
                    <div className="bg-[#1c1917] p-2 sm:p-4 rounded border-2 border-stone-800 shadow-[inset_0_10px_30px_rgba(0,0,0,0.8)] flex flex-col gap-4 relative h-full">
                      ...
                      <div className="grid grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-2 shrink-0 pb-4">
                        {gameState.rooms.map((room) => {
                          ...
                          const roomBackground = ROOM_BACKGROUNDS[room.type];
                          ...
                          <div
                            ...
                            style={roomBackground ? { backgroundImage: `url(${roomBackground})`, backgroundSize: 'cover', backgroundPosition: 'center' } : undefined}
                            ...
                            onMouseLeave={() => setHoveredRoomId(null)}
                          >
                            {roomBackground && <div className="absolute inset-0 bg-black/35 pointer-events-none z-0" />}
